...
try:
    ...
    from django.core.urlresolvers import reverse, get_urlconf, get_resolver # type: ignore
...
from filebrowser.decorators import make_user_dir_structure
...
class FileBrowserSite(object):
    ...
    def check_user_permissions(self, request, permission):
        # LADISPE function
        # Check the user privileges
        if not request.user.is_superuser and permission not in request.user.get_group_permissions():
            raise PermissionDenied()
        # Set the correct directory
        if request.user.is_superuser:
            self.directory = DIRECTORY
        else:
            self.directory = "{}Users/{}/".format(DIRECTORY, request.user.id)
            path = "{}".format(os.path.join(django.conf.settings.MEDIA_ROOT, self.directory))
            if not os.path.isdir(path):
                make_user_dir_structure(path)

    def browse(self, request):
        "Browse Files/Directories."
        self.check_user_permissions(request, 'filebrowser.view_filebrowser')
        filter_re = []
        for exp in EXCLUDE:
    ...
    def createdir(self, request):
        "Create Directory"
        self.check_user_permissions(request, 'filebrowser.add_filebrowser')
    ...
    def delete_confirm(self, request):
        ...
        self.check_user_permissions(request, 'filebrowser.delete_filebrowser')
        query = request.GET
        path = u'%s' % os.path.join(self.directory, query.get('dir', ''))
        ...
        if request.GET:
            try:
                ...
                if django.conf.settings.DEBUG:
                    print("[!] Can't delete a directory\n{}".format(e))
    ...
    def detail(self, request):
        ...
        """
        self.check_user_permissions(request, 'filebrowser.change_filebrowser')
